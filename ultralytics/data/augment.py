...
class BaseTransform:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image data and annotations. Expected keys include 'img_video_source' for
        ...
        Examples:
            >>> transform = BaseTransform()
            >>> labels = {"img_video_source": np.random.rand(640, 640, 3), "instances": []}
            >>> transformed_labels = transform(labels)
        """
...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        Examples:
            ...
            >>> result = transform({"image": img_video_source, "bboxes": boxes, "cls": classes})
        """
    ...
    def _mix_transform(self, labels):
        ...
        Examples:
            ...
            >>> labels = {"image": img_video_source, "bboxes": boxes, "mix_labels": [{"image": img2, "bboxes": boxes2}]}
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image and label information for the main (center) image.
                Must include 'img_video_source' key with the image array, and 'mix_labels' key with a list of two
        ...
        Returns:
            (Dict): A dictionary with the mosaic image and updated labels. Keys include:
                - 'img_video_source' (np.ndarray): The mosaic image array with shape (H, W, C).
                - Other keys from the input labels, updated to reflect the new image dimensions.

        Examples:
            >>> mosaic = Mosaic(dataset, imgsz=640, p=1.0, n=3)
            >>> labels = {
            ...     "img_video_source": np.random.rand(480, 640, 3),
            ...     "mix_labels": [{"img_video_source": np.random.rand(480, 640, 3)} for _ in range(2)],
            ... }
            >>> result = mosaic._mosaic3(labels)
            >>> print(result["img_video_source"].shape)
            (640, 640, 3)
        """
        mosaic_labels = []
        s = self.imgsz
        for i in range(3):
            ...
            # Load image
            img = labels_patch["img_video_source"]
            h, w = labels_patch.pop("resized_shape")

            # Place img_video_source in img3
            if i == 0:  # center
        ...
        final_labels["img_video_source"] = img3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        Returns:
            (Dict): A dictionary containing the mosaic image and updated labels. The 'img_video_source' key contains the mosaic
        ...
        Examples:
            ...
            >>> labels = {
            ...     "img_video_source": np.random.rand(480, 640, 3),
            ...     "mix_labels": [{"img_video_source": np.random.rand(480, 640, 3)} for _ in range(3)],
            ... }
            >>> result = mosaic._mosaic4(labels)
            >>> assert result["img_video_source"].shape == (1280, 1280, 3)
        """
        mosaic_labels = []
        s = self.imgsz
        ...
        for i in range(4):
            ...
            # Place img_video_source in img4
            if i == 0:  # top left
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img_video_source"] = img4
        return final_labels
    ...
    def _mosaic9(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the input image and its associated labels. It should have
                the following keys:
                - 'img_video_source' (numpy.ndarray): The input image.
        ...
        Returns:
            (Dict): A dictionary containing the mosaic image and updated labels. It includes the following keys:
                - 'img_video_source' (numpy.ndarray): The final mosaic image.
        ...
        Examples:
            ...
            >>> mosaic_image = mosaic_result["img_video_source"]
        """
        mosaic_labels = []
        s = self.imgsz
        ...
        for i in range(9):
            ...
            # Place img_video_source in img9
            if i == 0:  # center
        ...
        final_labels["img_video_source"] = img9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _update_labels(labels, padw, padh):
        ...
        Examples:
            >>> labels = {"img_video_source": np.zeros((100, 100, 3)), "instances": Instances(...)}
            >>> padw, padh = 50, 50
            >>> updated_labels = Mosaic._update_labels(labels, padw, padh)
        """
        nh, nw = labels["img_video_source"].shape[:2]
...
class MixUp(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Examples:
            ...
            >>> dataset = YOLODataset("path/to/taozi.yaml")
    ...
    def _mix_transform(self, labels):
        ...
        labels["img_video_source"] = (labels["img_video_source"] * r + labels2["img_video_source"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    Examples:
        ...
        >>> labels = {"img_video_source": image, "cls": np.array([0, 1]), "instances": Instances(...)}
        >>> result = transform(labels)
        >>> transformed_image = result["img_video_source"]
    ...
    def affine_transform(self, img, border):
        ...
        Examples:
            >>> import numpy as np
            >>> img_video_source = np.random.rand(100, 100, 3)
            >>> border = (10, 10)
            >>> transformed_img, matrix, scale = affine_transform(img_video_source, border)
        """
        # Center
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image data and annotations.
                Must include:
                    'img_video_source' (ndarray): The input image.
        ...
        Returns:
            (Dict): Transformed labels dictionary containing:
                - 'img_video_source' (np.ndarray): The transformed image.
        ...
        Examples:
            ...
            >>> image = np.random.randint(0, 255, (640, 640, 3), dtype=np.uint8)
            >>> labels = {
            ...     "img_video_source": image,
            ...     "cls": np.array([0, 1, 2]),
            ...     "instances": Instances(bboxes=np.array([[10, 10, 50, 50], [100, 100, 150, 150]])),
            ... }
            >>> result = transform(labels)
            >>> assert result["img_video_source"].shape[:2] == result["resized_shape"]
        """
        if self.pre_transform and "mosaic_border" not in labels:
            labels = self.pre_transform(labels)
        labels.pop("ratio_pad", None)  # do not need ratio pad

        img = labels["img_video_source"]
        cls = labels["cls"]
...
class RandomHSV:
    ...
    Examples:
        ...
        >>> labels = {"img_video_source": image}
        >>> augmented_labels = augmenter(labels)
        >>> augmented_image = augmented_labels["img_video_source"]
    """
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image data and metadata. Must include an 'img_video_source' key with
                the image as a numpy array.

        Returns:
            (None): The function modifies the input 'labels' dictionary in-place, updating the 'img_video_source' key
                with the HSV-augmented image.

        Examples:
            >>> hsv_augmenter = RandomHSV(hgain=0.5, sgain=0.5, vgain=0.5)
            >>> labels = {"img_video_source": np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)}
            >>> hsv_augmenter(labels)
            >>> augmented_img = labels["img_video_source"]
        """
        img = labels["img_video_source"]
...
class RandomFlip:
    ...
    Examples:
        ...
        >>> result = transform({"img_video_source": image, "instances": instances})
        >>> flipped_image = result["img_video_source"]
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the following keys:
                'img_video_source' (numpy.ndarray): The image to be flipped.
        ...
        Returns:
            (Dict): The same dictionary with the flipped image and updated instances:
                'img_video_source' (numpy.ndarray): The flipped image.
                'instances' (ultralytics.utils.instance.Instances): Updated instances matching the flipped image.

        Examples:
            >>> labels = {"img_video_source": np.random.rand(640, 640, 3), "instances": Instances(...)}
        ...
        labels["img_video_source"] = np.ascontiguousarray(img)
...
class LetterBox:
    ...
    Examples:
        ...
        >>> result = transform(labels)
        >>> resized_img = result["img_video_source"]
    ...
    def __call__(self, labels=None, image=None):
        ...
        Examples:
            ...
            >>> result = letterbox(labels={"img_video_source": np.zeros((480, 640, 3)), "instances": Instances(...)})
            >>> resized_img = result["img_video_source"]
            >>> updated_instances = result["instances"]
        """
        if labels is None:
            labels = {}
        img = labels.get("img_video_source") if image is None else image
    ...
    def _update_labels(self, labels, ratio, padw, padh):
        ...
        """
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(*labels["img_video_source"].shape[:2][::-1])
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing:
                - 'img_video_source' (np.ndarray): The image to augment.
        ...
        Returns:
            (Dict): Dictionary with augmented image and updated instances under 'img_video_source', 'cls', and 'instances' keys.

        Examples:
            >>> labels = {"img_video_source": np.random.rand(640, 640, 3), "cls": np.array([0, 1, 2]), "instances": Instances(...)}
            >>> augmenter = CopyPaste(p=0.5)
            >>> augmented_labels = augmenter(labels)
        """
        im = labels["img_video_source"]
        cls = labels["cls"]
        h, w = im.shape[:2]
        instances = labels.pop("instances")
        instances.convert_bbox(format="xyxy")
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image data and annotations. Expected keys are:
                - 'img_video_source': numpy.ndarray representing the image
        ...
        Examples:
            >>> transform = Albumentations(p=0.5)
            >>> labels = {
            ...     "img_video_source": np.random.rand(640, 640, 3),
            ...     "cls": np.array([0, 1]),
            ...     "instances": Instances(bboxes=np.array([[0, 0, 1, 1], [0.5, 0.5, 0.8, 0.8]])),
            ... }
            >>> augmented = transform(labels)
            >>> assert augmented["img_video_source"].shape == (640, 640, 3)
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img_video_source"] = new["image"]
                    labels["cls"] = np.array(new["class_labels"])
                    bboxes = np.array(new["bboxes"], dtype=np.float32)
                labels["instances"].update(bboxes=bboxes)
        else:
            labels["img_video_source"] = self.transform(image=labels["img_video_source"])["image"]  # transformed
...
class Format:
    ...
    Examples:
        ...
        >>> img_video_source = formatted_labels["img_video_source"]
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image and annotation data with the following keys:
                - 'img_video_source': The input image as a numpy array.
                - 'cls': Class labels for instances.
                - 'instances': An Instances object containing bounding boxes, segments, and keypoints.

        Returns:
            (Dict): A dictionary with formatted data, including:
                - 'img_video_source': Formatted image tensor.
        ...
        Examples:
            >>> formatter = Format(bbox_format="xywh", normalize=True, return_mask=True)
            >>> labels = {"img_video_source": np.random.rand(640, 640, 3), "cls": np.array([0, 1]), "instances": Instances(...)}
            >>> formatted_labels = formatter(labels)
            >>> print(formatted_labels.keys())
        """
        img = labels.pop("img_video_source")
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        instances = labels.pop("instances")
        ...
        if self.return_mask:
            ...
            labels["masks"] = masks
        labels["img_video_source"] = self._format_img(img)
    ...
    def _format_img(self, img):
        ...
        Examples:
            >>> import numpy as np
            >>> img_video_source = np.random.rand(100, 100, 3)
            >>> formatted_img = self._format_img(img_video_source)
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING ⚠️ No 'flip_idx' array defined in taozi.yaml, setting augmentation 'fliplr=0.0'")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f"taozi.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
...
def classify_transforms(
    ...
    Examples:
        ...
        >>> img_video_source = Image.open("path/to/image.jpg")
        >>> transformed_img = transforms(img_video_source)
    """
...
class ClassifyLetterBox:
    ...
    Examples:
        ...
        >>> img_video_source = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
        >>> result = transform(img_video_source)
        >>> print(result.shape)
        (640, 640, 3)
    """
...
class CenterCrop:
    ...
    def __init__(self, size=640):
        ...
        Examples:
            ...
            >>> img_video_source = np.random.rand(300, 300, 3)
            >>> cropped_img = transform(img_video_source)
...
class ToTensor:
    ...
    Examples:
        ...
        >>> img_video_source = np.random.randint(0, 255, (640, 640, 3), dtype=np.uint8)
        >>> tensor_img = transform(img_video_source)
