...
class Detect(nn.Module):
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            ...
            b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img_video_source)
        if self.end2end:
...
class WorldDetect(Detect):
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            a[-1].bias.data[:] = 1.0  # box
            # b[-1].bias.data[:] = math.log(5 / m.nc / (640 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img_video_source)
