...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        # Load a model
        model = YOLO("yolov8n_input6.yaml")  # build a new model from scratch
...
# Set logger
LOGGER = set_logging(LOGGING_NAME, verbose=VERBOSE)  # define globally (used in predict.py, val.py, predict.py, etc.)
...
def yaml_save(file="taozi.yaml", data=None, header=""):
    """
    Save YAML data to a file.

    Args:
        file (str, optional): File name. Default is 'taozi.yaml'.
...
def yaml_load(file="taozi.yaml", append_filename=False):
    """
